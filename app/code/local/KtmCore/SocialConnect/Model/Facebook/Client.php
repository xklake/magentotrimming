...
class KtmCore_SocialConnect_Model_Facebook_Client
{
    const REDIRECT_URI_ROUTE = 'socialconnect/facebook/connect';

    const XML_PATH_ENABLED = 'customer/ktmcore_socialconnect_facebook/enabled';
    const XML_PATH_CLIENT_ID = 'customer/ktmcore_socialconnect_facebook/client_id';
    const XML_PATH_CLIENT_SECRET = 'customer/ktmcore_socialconnect_facebook/client_secret';
    ...
    protected function fetchAccessToken()
    {
        if(empty($_REQUEST['code'])) {
            throw new Exception(
                Mage::helper('ktmcore_socialconnect')
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401)) {
                if(isset($decoded_response->error->message)) {
                    ...
                    $message = Mage::helper('ktmcore_socialconnect')
                        ->__('Unspecified OAuth error occurred.');
                }

                throw new KtmCore_SocialConnect_FacebookOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('ktmcore_socialconnect')
...
class KtmCore_SocialConnect_FacebookOAuthException extends Exception
{}