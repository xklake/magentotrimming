 ...
 */
class KtmCore_AjaxSwatches_Helper_Productimg extends Mage_ConfigurableSwatches_Helper_Productimg
{
    ...
    public function indexProductImages($product, $preValues = null)
    {
        ...
        if (!isset($this->_productImagesByLabel[$product->getId()])) {
            ...
            foreach ($searchValues as $label) {
				...
				if(isset($mapping[$label])){ //ktmcore add-on - after skipping the !isSalable colors, we don't have all the labels available
    ...
    public function filterImageInGallery($product, $image)
    {
        ...
        if (!isset($this->_productImageFilters[$product->getId()])) {
			...
			/* KtmCore: this goes out: */
			//$filters = array_unique($mapping['labels']);
			
			/* KtmCore: and this comes in (that's because we changed the labels object to include sort_ids) */
            $filters = array();
